...
def page_record(page: NotionPage, workspace_id: str, workspace_name: str) -> SourceRecord:
    ...
    return SourceRecord(
        ...
        metadata={"parent_page_id": page.parent_page_id, "pipeline_version": 1},
        access=SourceAccess(policy_version="notion-oauth-v1"),
    )
...
def write_page(
    ...
    edges = [_edge(graph, "CONTAINS", "Workspace", "Document", root_uid, uid, record.record_key)]
    if page.parent_page_id:
        ...
        edges.append(_edge(graph, "PARENT_OF", "Document", "Document",
                           parent_uid, uid, record.record_key))
